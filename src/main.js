...
const posts = [
  {
    ...
    excerpt:
      "Những điều mình ước đã biết khi bắt đầu học HTML, CSS và JavaScript.",
  ...
  },
  {
    ...
    excerpt:
      "Cá tính không đến từ thật nhiều hiệu ứng — nó đến từ những quyết định nhất quán.",
  ...
  },
  {
    ...
    excerpt:
      "Ghi chép ngắn về việc biến một ý tưởng lớn thành từng việc nhỏ mỗi ngày.",
  ...
  },
  {
    ...
    excerpt:
      "Lưu theme, bài yêu thích và trạng thái giao diện mà chưa cần backend.",
...
document.querySelector("#app").innerHTML = `
  <main class="desktop ${state.theme === "light" ? "light" : ""}" aria-label="Màn hình H.T. Blog">
      ...
      <div class="window-body p-5 sm:p-7">
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 class="max-w-xl text-3xl font-black leading-tight sm:text-5xl">Chia sẻ những cách học, thiết kế và hành trình sáng tạo.</h1>
          </div>
    ...
    <section id="about" class="window glass is-hidden" style="left:28%;top:14%;z-index:6" aria-label="Cửa sổ giới thiệu">
      ...
      <div class="window-body p-6 sm:p-8">
        <div class="grid gap-7 sm:grid-cols-[150px_1fr] sm:items-start">
          ...
          <div><p class="text-sm font-bold uppercase tracking-[.2em] text-teal-300">Xin chào, mình là H.T.</p><h2 class="mt-2 text-3xl font-black">Một sinh viên đang theo đuổi ngành lập trình website</h2><p class="muted mt-4 leading-7 text-slate-300">Đây là một trang web nhỏ mà mình thử nghiệm, thiết kế web và quá trình hoàn thành những dự án cá nhân.</p><div class="mt-5 flex flex-wrap gap-2">${["JavaScript", "Tailwind CSS", "UI Design", "Drawing"].map((x) => `<span class="soft-surface rounded-full border border-white/15 bg-white/8 px-3 py-1.5 text-sm">${x}</span>`).join("")}</div></div>
        </div>
      </div>
    </section>
    ...
    <section id="projects" class="window glass is-hidden" style="left:22%;top:11%;z-index:6" aria-label="Cửa sổ dự án">
      ...
      <div class="window-body p-6 sm:p-8"><p class="mb-5 text-sm font-bold uppercase tracking-[.2em] text-teal-300">Selected work</p><div class="grid gap-4 sm:grid-cols-2">${[
        ["Gaming", "Website mô phỏng trò chơi nhỏ cấu hình 2D", "01"],
        [
          "Music Player",
          "Trình phát nhạc có playlist và thanh kéo tương tác.",
          "02",
        ],
        ["Blog", "Desktop blog với cửa sổ kéo thả.", "03"],
      ]
        .map(
          (p) =>
            `<article class="soft-surface rounded-3xl border border-white/15 bg-white/8 p-5"><span class="text-sm text-teal-300">${p[2]}</span><h3 class="mt-7 text-2xl font-bold">${p[0]}</h3><p class="muted mt-2 text-slate-300">${p[1]}</p></article>`,
        )
        .join("")}</div></div>
    </section>
    ...
    <section id="contact" class="window glass is-hidden" style="left:32%;top:18%;z-index:6" aria-label="Cửa sổ liên hệ">
      ...
      <div class="window-body p-7 text-center sm:p-10"><div class="mx-auto grid size-20 place-items-center rounded-3xl bg-teal-300 text-4xl text-slate-950">✉</div><h2 class="mt-5 text-3xl font-black">Cùng tạo điều gì đó thú vị.</h2><p class="muted mx-auto mt-3 max-w-md text-slate-300">Bạn có câu hỏi, góp ý hoặc muốn trao đổi về một dự án? Hãy gửi email cho mình.</p><a href="mailto:tuancl12345678@gmail.com" class="mt-6 inline-flex rounded-full bg-teal-300 px-6 py-3 font-bold text-slate-950 transition hover:bg-teal-200">tuancl12345678@gmail.com</a></div>
    </section>
...
function renderCategories() {
  ...
  document.querySelector("#categories").innerHTML = categories
    .map(
      (category) =>
        `<button class="tag rounded-full border border-white/15 px-3 py-1.5 text-sm font-semibold ${category === state.category ? "active" : "soft-surface bg-white/8"}" data-category="${category}">${category}</button>`,
    )
    .join("");
}
...
function renderPosts() {
  ...
  const visible = posts.filter(
    (post) =>
      (state.category === "Tất cả" || post.category === state.category) &&
      (!query || `${post.title} ${post.excerpt}`.toLowerCase().includes(query)),
  );
  document.querySelector("#posts").innerHTML = visible.length
    ? visible
        .map(
          (post) => `
    <button class="post-card soft-surface overflow-hidden rounded-3xl border border-white/15 bg-white/5 text-left" data-post="${post.id}">
      ...
      <span class="block p-5"><span class="muted text-xs font-semibold uppercase tracking-wider text-slate-400">${post.date} · ${post.readTime}</span><strong class="mt-2 block text-xl leading-snug">${post.title}</strong><span class="muted mt-2 block text-sm leading-6 text-slate-300">${post.excerpt}</span></span>
        ...
        .join("")
    : `<div class="soft-surface col-span-full rounded-3xl border border-white/15 bg-white/5 p-10 text-center"><p class="text-3xl">⌕</p><h3 class="mt-3 text-xl font-bold">Không tìm thấy bài viết</h3><p class="muted mt-1 text-slate-300">Thử một từ khóa hoặc danh mục khác nhé.</p></div>`;
}
...
function openPost(id) {
  ...
  document.querySelector("#reader-content").innerHTML =
    `<p class="text-sm font-bold uppercase tracking-[.2em] text-teal-300">${post.category} · ${post.date}</p><h2 class="mt-3 text-3xl font-black leading-tight sm:text-5xl">${post.title}</h2><p class="muted mt-4 text-lg text-slate-300">${post.excerpt}</p><div class="my-7 h-px bg-white/15"></div><div class="muted space-y-5 text-base leading-8 text-slate-200">${post.content
      .split("\n\n")
      .map((p) => `<p>${p}</p>`)
      .join("")}</div>`;
  openWindow("reader");
...
document.addEventListener("click", (event) => {
  ...
  if (close || minimize)
    document
      .querySelector(
        `#${(close || minimize).dataset[close ? "close" : "minimize"]}`,
      )
      .classList.add("is-hidden");
  if (maximize)
    document
      .querySelector(`#${maximize.dataset.maximize}`)
...
document.querySelector("#theme").addEventListener("click", (event) => {
  ...
  document
    ...
    .classList.toggle("light", state.theme === "light");
